...
# 💡 追加：システム全体の「最終更新日時」を記録する変数（初期状態は起動時、または未更新）
last_updated = None

@app.route("/", methods=["GET", "POST"])
def index():
    global next_id, last_updated  # 💡 last_updated を書き換えるため global 宣言を追加
    
    # 【各種処理】ボタンが押されてデータが送られてきた場合（POST）
    if request.method == "POST":
        action = request.form.get("action")
        data_changed = False  # 💡 データの変更（更新・追加）があったかを判定するフラグ
        ...
        if action == "update":
            ...
            for key in list(inventory.keys()):
                ...
                if key in inventory:
                    if (inventory[key]["name"] != name or 
                        inventory[key]["stock"] != stock or 
                        inventory[key]["threshold"] != threshold):
                        data_changed = True
        ...
        # 2. 新しい商品の追加処理
            ...
            if add_name:
                ...
                next_id += 1
                data_changed = True  # 新規追加があったのでフラグを立てる
                
        # 💡 追加：データの変更があった場合のみ、最終更新日時を現在時刻（時分秒まで）で更新
        if data_changed:
            last_updated = datetime.now().strftime("%Y年%m月%d日 %H時%M分%S秒")
    ...
    # 現在のサーバー日付を取得し、「◯年◯月◯日」の形式にフォーマット
